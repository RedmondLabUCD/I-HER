...
class FetchReachEnsemble(gym.Env):
    def __init__(self, ensemble, env, obs_norm, clip_range, close_gripper=False):
    ...
    def step(self, action):
        ...
        if self.close_gripper:
            ...
            action[-1] = -1
        with torch.no_grad():
            # Normalise obs before and de-normalise after
            ...
            prediction = self.ensemble[i](torch.tensor(obs).float(), torch.tensor(action).float())
            prediction = (prediction.detach().numpy().squeeze() * self.obs_norm.std) + self.obs_norm.mean

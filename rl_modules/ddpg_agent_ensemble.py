...
class ddpg_agent_ensemble:
    ...
    def learn(self):
        ...
        """
        print('beginning ensemble learning')
        # torch.autograd.set_detect_anomaly(True)
        # self.test_dynamics()
        # print('learn')
        # self.collect_real_exp(0, rollouts=100)
        # self.update_dynamics(0, steps=100)
        # # self.test_dynamics()
        # for i in range(len(self.ensemble_models)):
        #     print('\nMPI {}, Model {}, mean weight: {}'.format(MPI.COMM_WORLD.Get_rank(), i, np.mean(self.ensemble_models[i].fc1.weight.detach().numpy())))
        #     print('MPI {}, Env Model {}, mean weight: {}'.format(MPI.COMM_WORLD.Get_rank(), i, np.mean(self.env.ensemble[i].fc1.weight.detach().numpy())))
        # print('MPI {}, obs means: {}'.format(MPI.COMM_WORLD.Get_rank(),self.real_o_norm.mean))
        # return
        ...
        for epoch in range(self.args.n_epochs):
            ...
            if MPI.COMM_WORLD.Get_rank() == 0:
                ...
                torch.save([self.o_norm.mean, self.o_norm.std, self.g_norm.mean, self.g_norm.std, self.actor_network.state_dict()], \
                            self.model_path + '/modeltestdynamics{}.pt'.format(epoch))
    ...
    def get_intrinsic_reward(self, obs, action, scale=2, clip=0.8):
        ...
        return np.clip(r_intrinsic/scale, 0, clip) # TODO: make as arg hyperparameter
    ...
    def update_dynamics(self, epoch, batch_size=512, steps=2e4):
        ...
        if MPI.COMM_WORLD.Get_rank() == 0:
            ...
            torch.save(model_state, 'testEnsembleOf{}_epoch{}.tar'.format(len(self.ensemble_models), epoch))

    def collect_real_exp(self, epoch, rollouts=512):
        mb_obs = []
        rollouts = int(rollouts/MPI.COMM_WORLD.Get_size())
        for _ in range(rollouts):
            # # Randomly pick one of last 5 actors to collect
            # if epoch > 1:
            #     actor_idx = len(self.old_actors) - np.random.randint(1, self.update_dynamics_every+1)
            #     old_actor = self.old_actors[actor_idx]
            #     actor = old_actor['actor']
            # reset the environment

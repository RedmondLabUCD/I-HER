...
class ddpg_real_transfer:
    def __init__(self, args, env, env_params):
        ...
        model_path = 'pickplace_actorcritic.pt'
    ...
    def learn(self):
        ...
        """
        print('Training PickPlace agent in Push task')
        ...
        for epoch in range(self.args.n_epochs):
            # if epoch % 20 == 0:
            #     print('Buffer current size: {}'.format(self.buffer.current_size))
            #     # self.collect_real_exp()
            #     # self.update_mdrnn()
            ...
            if MPI.COMM_WORLD.Get_rank() == 0:
                ...
                torch.save([self.o_norm.mean, self.o_norm.std, self.g_norm.mean, self.g_norm.std, self.actor_network.state_dict()], \
                            self.model_path + '/realtransfer.pt')
    ...
    #     self.mdrnn.train()
    #     batch_size = 10 # TODO: make arg
    #     steps = 1000
    #     cum_loss = 0
    #     verbose = False
    #     for i in range(steps):
    #         # print('RNN weights max: {:.3f}, {:.3f}, min: {:.3f}, {:.3f}'.format(np.amax(self.mdrnn.rnn.weight_ih_l0.squeeze().detach().numpy()), np.amax(self.mdrnn.rnn.weight_hh_l0.squeeze().detach().numpy()), np.amin(self.mdrnn.rnn.weight_ih_l0.squeeze().detach().numpy()), np.amin(self.mdrnn.rnn.weight_hh_l0.squeeze().detach().numpy())))
    #         # print('GMM weights max: {:.3f}, min: {:.3f}'.format(np.amax(self.mdrnn.gmm_linear.weight.squeeze().detach().numpy()), np.amin(self.mdrnn.gmm_linear.weight.squeeze().detach().numpy())))
    #         batch = self.buffer_real.mdrnn_sample(batch_size)
    #         obs, action, next_obs = batch['obs'], batch['actions'], batch['obs_next']
            
    #         # meanobs = np.mean(obs, axis=0)
    #         # meanobs = np.mean(meanobs, axis=0)
    #         # stdobs = np.std(obs, axis=0)
    #         # stdobs = np.std(stdobs, axis=0)
    #         # for j in range(meanobs.shape[0]):
    #         #     print('Obs {}, mean: {}, std: {}'.format(j, meanobs[j], stdobs[j]))
    #         # break
    #         reward = torch.zeros(obs.shape[0],obs.shape[1])
    #         terminal = torch.zeros(obs.shape[0],obs.shape[1])
    #         print('Obs min: {:.3f}, max: {:.3f}, mean: {:.3f}, std: {:.3f}'.format(np.amin(obs), np.amax(obs), np.mean(obs), np.std(obs)))
    ...
    #         losses.backward()
    #         # torch.nn.utils.clip_grad_norm_(self.mdrnn.parameters(), 0.5)
    #         nn.utils.clip_grad_value_(self.mdrnn.parameters(), clip_value=1.0)
    #         # print('weight_ih grad max: {}, min: {}'.format(np.amax(self.mdrnn.rnn.weight_ih_l0.grad.numpy()), np.amin(self.mdrnn.rnn.weight_ih_l0.grad.numpy())))
    #         # print('weight_hh grad max: {}, min: {}'.format(np.amax(self.mdrnn.rnn.weight_hh_l0.grad.numpy()), np.amin(self.mdrnn.rnn.weight_hh_l0.grad.numpy())))
    #         self.mdrnn_optimizer.step()
    #         cum_loss += losses
    #         print('step {} loss: {}\n'.format(i, losses))
    #     print('MDRNN loss: {}'.format(cum_loss/steps))
    #     # self.mdrnn_scheduler.step(cum_loss/steps)
    ...
    # def get_mdrnn_loss(self, obs, action, reward, terminal, next_obs, include_reward: bool, verbose=False):
    #     obs, action,\
    #         reward, terminal,\

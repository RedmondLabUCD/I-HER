...
"""
class ddpg_agent_simpledynamics:
    def __init__(self, args, env, env_params):
        ...
        model_state = torch.load('dynamicsmodel_norm.tar')
    ...
    def learn(self):
        ...
        # self.test_dynamics()
        # return
        if MPI.COMM_WORLD.Get_rank() == 0: print('Beginning simple pretrained imaginary learning...')
        ...
        for epoch in range(self.args.n_epochs):
            ...
            if MPI.COMM_WORLD.Get_rank() == 0:
                print('[{}] epoch is: {}, real rate: {:.3f}, imag rate: {:.3f}'.format(datetime.now(), epoch, real_success_rate, imag_success_rate))
                torch.save([self.o_norm.mean, self.o_norm.std, self.g_norm.mean, self.g_norm.std, self.actor_network.state_dict()], \
                            self.model_path + '/acmagicNp2-{}.pt'.format(epoch))
    ...
    #     batch_size = 512 # TODO: make arg
    #     steps = int(1e4)
    #     cum_loss = 0
    #     for i in range(steps):
    #         batch = self.buffer_real.sample_batch(batch_size)
    ...
    #         torch.save(model_state, 'dynamicsmodel4_norm{}.tar'.format(epoch))
    ...
    # do the evaluation
    def _eval_agent(self, real=True):
        if real:
            env = self.realenv
        else:
            env = self.env
